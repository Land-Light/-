...
def decide_tensakit_marks(
    ...
    main_instr = (
        ...
        "  次に採点基準の各減点項目について、『その項目が要求する内容・要素・訳語・書き下しが"
        "答案に明確に書けているか』を1つずつ厳密に確認する。手順は必ず: (1)採点基準の模範解答と"
        "各減点項目を読む→(2)答案の記述を読み取る→(3)各減点項目ごとに『要求内容が明確にあるか』を○×判定。"
        "×(明確に書かれていない・言及なし・曖昧・訳や書き下しに誤りがある・要素が欠落)なら、"
        "必ずその番号を deduct_numbers に入れる(例[2,4])。\n"
        "  ★重要: 実際の答案は模範解答と細部が異なり、多くの設問で何らかの減点が生じる。"
        "安易に『減点なし(満点)』にしないこと。各減点項目を模範解答と照合し、欠落・誤り・言い換え不足を"
        "見逃さない。根拠のある減点だけを入れる(該当が無ければ空でよいが、確認は必ず全項目行う)。"
