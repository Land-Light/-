...
def decide_tensakit_marks(
    ...
    content.append({
        ...
        "text": (
            ...
            "この採点は『まず加点項目(+N 配点=満点)を与え、答案の誤りに応じて減点項目(-1)を引く』方式です。"
            "上の答案と採点基準(参照実例)に照らし、各セクションについて次を判断してください。\n"
            "・記述・説明など配点方式の設問: 原則 add_indices に加点項目(満点)を入れて満点を基準にし、"
            "採点基準の減点項目に照らして、該当する誤り・不足の減点番号を deduct_numbers にすべて列挙する"
            "(例: 減点項目の2と4が該当 → deduct_numbers=[2,4])。白紙や大きく的外れな答案は加点しない(add空)。\n"
            "・記号選択(生徒の回答: ア〜ク・未回答 等のラジオ)の設問: 答案の該当する解答欄(記号 a,b,c… や"
            "小問番号で対応)を見て、生徒が実際に書いた記号を読み取り、その選択肢 index を radio_index に1つ入れる。"
            "はっきり記入があるのに『未回答』を選ばない。明らかに空欄のときだけ『未回答』にする。\n"
            "・漢字書き取り等: 正解した小問の加点項目のみ add_indices に入れる。\n"
            "減点番号は採点基準の減点項目の番号(1,2,3…)に対応させること。コメントは扱いません。"
            "該当が無い項目は空/ null のまま返すこと。\n\n"
            + panel_text
        ),
    })
