...
- **学習済み採点基準** — 東京都立大(2023〜2025)・奈良女子大(2020〜2025)・
  埼玉大(2018〜2025)・千葉大(2016〜2024)の過去問演習講座「解答・採点基準」
  計36試験分を参照コーパスとして搭載
- **PDF出力** — 添削結果レポートをワンクリックでPDFダウンロード(日本語フォント埋め込み)
- **答案への直接書き込み** — スキャン答案PDFに赤ペン添削(設問上の得点、記号、
  欄外の縦書き講評、得点欄・採点者欄)をオーバーレイ合成(`annotator.py`)
- **スキャン答案の一括採点** — 答案用紙のスキャンPDFを**複数まとめてアップロード**すると、
  AIが手書きを判読→出典判別→採点し、書き込み済みPDF+レポートPDFを返却(`scan_grader.py`)。
  一括ZIPダウンロード対応。1枚の失敗が他の答案の採点を止めない
...
すべての設問に必ずいずれかの記号を付ける(マーク漏れを作らない)。
この規約は `annotator.py`(`mark_kind_for()`)と `grader.py` のシステムプロンプトの
両方に組み込まれている。
...
| `annotator.py` | スキャン答案への赤ペン書き込み(ローカル・API不要) |
